...
cnfrm = "bash -c 'echo -e \"No\nYes\" | dmenu -p Quit'"

quit = function()
	awful.spawn.easy_async(cnfrm, function(out)
		if out:sub(1, 3) == "Yes" then
			awesome.quit()
		end
	end)
end
...
tags = {
        names = { "T(1)", "T(2)", "T(3)", "T(4)", "T(5)", "F(6)", "F(7)", "F(8)", "T(9)" },
            layout = { awful.layout.layouts[2], awful.layout.layouts[2], awful.layout.layouts[2], 
						awful.layout.layouts[2], awful.layout.layouts[2], awful.layout.layouts[1],
						awful.layout.layouts[1], awful.layout.layouts[1], awful.layout.layouts[2]}
}
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            layout = wibox.layout.fixed.horizontal,
			awful.widget.watch('bash -c "/home/louis/.config/awesome/update.sh"', 15),
            mykeyboardlayout,
...
globalkeys = gears.table.join(
    ...
    awful.key({  }, "XF86Launch2",
            function ()
                awful.util.spawn("bash -c 'export LC_TIME=en_CA.UTF-8 ; thunderbird'")
            end),

    awful.key({  }, "XF86Launch3",
            function ()
                awful.util.spawn("riot-web")
            end),
    awful.key({  }, "XF86Launch5",
            function ()
                awful.util.spawn("veracrypt --background-task /home/louis/secure.vc")
            end),
	...
	awful.key({ }, "XF86MonBrightnessDown",
            function ()
                awful.util.spawn("/home/louis/.local/scripts/brightness down")
            end),
	awful.key({ }, "XF86MonBrightnessUp",
            function ()
                awful.util.spawn("/home/louis/.local/scripts/brightness up")
            end),
	awful.key({ }, "XF86PowerOff",
            function ()
                awful.util.spawn("sh -c \"xscreensaver-command -lock;sleep 1;sudo pm-suspend;xscreensaver-command -deactivate\"")
            end),
...
clientkeys = gears.table.join(
	...
	awful.key({ modkey, "Control" }, "f",
            function (c)
				if c.screen.mywibox.visible then
...
os.execute("killall -9 indicator-kdeconnect;/usr/bin/indicator-kdeconnect &")
os.execute("killall redshift;/usr/bin/gtk-redshift -l 45.572266:-71.999318 &")
os.execute("killall -9 owncloud;/usr/bin/owncloud &")
os.execute("killall -9 nextcloud;/usr/bin/nextcloud &")
os.execute("killall -9 blueman-applet;/usr/bin/blueman-applet &")
os.execute("killall -9 caffeine-indicator;/usr/bin/caffeine-indicator &")

os.execute("if ps aux | grep pcmanfm | grep -v grep; then :;else pcmanfm -d; fi &")

os.execute("setxkbmap ca,fr ,bepo")
-- }}}
